...
export function parseCurlCommand(curlCommand: string): ParsedCurlCommand | null {
  ...
  // Track flags that affect method inference and pre-scan tokens for explicit method hints
  ...
  for (let j = 1; j < tokens.length; j++) {
    ...
    if (t === '-X' || t === '--request') {
      ...
      const mt = t.substring(2).toUpperCase();
      if (mt) {
        result.method = mt;
        explicitMethod = true;
      }
      ...
      // Consume the file argument if present
      if (!explicitMethod) {
        result.method = 'PUT';
      }
      if (j + 1 < tokens.length) {
  ...
  while (i < tokens.length) {
    ...
    if (token === '-X' || token === '--request') {
      ...
      // Combined short option, e.g. -XPOST
      const methodToken = token.substring(2).toUpperCase();
      if (methodToken) {
      ...
      // Force GET even when data is provided
      getFlag = true;
      result.method = 'GET';
      ...
      if (i < tokens.length) {
        ...
        // If method wasn't explicitly set and we have data, assume POST unless -G/--get was used
        if (!explicitMethod && result.method === 'GET' && !getFlag) {
          result.method = 'POST';
        }
      }
      ...
      // Upload file implies PUT if not explicitly set
      i++;
      if (!explicitMethod) {
        result.method = 'PUT';
      }
